...
void MainWindow::setupLoggers() {
  ...
  auto basic_file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
      QDir::tempPath().toStdString() + "/logs-client/basic-log.txt", true);
  ...
  auto warn_file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
      QDir::tempPath().toStdString() + "/logs-client/warns-log.txt", true);
  ...
  auto error_file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
      QDir::tempPath().toStdString() + "/logs-client/errors-log.txt", true);
  ...
  auto logger = std::make_shared<spdlog::logger>(
      "client_logger", logger_sinks.begin(), logger_sinks.end());
  logger->set_level(spdlog::level::trace);

  // use warn_logger only for "warning-log.txt"
  auto warn_logger = std::make_shared<spdlog::logger>(
      "client_warn_logger", warn_logger_sinks.begin(), warn_logger_sinks.end());
...
void MainWindow::showData() {
  ...
  // total size labels
  ui->totalSize_label->setText(
      tr("Total size:\t") +
      QString::number(currentData["total"].get<quint64>() / 1e6) + tr(" Mb"));
  ...
  for (auto &file : currentData["files"]) {
    ...
    ui->tableWidget->setItem(
        k, 0, new QTableWidgetItem(QString::fromStdString(fileName)));
    ui->tableWidget->setItem(
        k, 1, new QTableWidgetItem(QString::fromStdString(createdOn)));
    ui->tableWidget->setItem(k, 2,
                             new QTableWidgetItem(QString::number(fileSize)));
...
bool MainWindow::getData() {
  ...
  if (currentPath.isEmpty()) {
    ...
    QMessageBox::warning(this, "No path given",
                         "Set the path before proceeding");
    return false;
  }

  client_logger->info("The path has been succesfully set to '{}'",
                      currentPath.toStdString());
  for (auto el = defaultExtensionsMap.begin(); el != defaultExtensionsMap.end();
       el++) {
    if (el.key()->isChecked()) {
  ...
  std::string request =
      formRequest(this->currentPath, this->extensions);
...
std::string MainWindow::formRequest(QString dirpath, QSet<QString> extensions)
{
    client_logger->debug("Forming request...");

    QString request = QString("%1\n%2").arg(dirpath, extensions.values().join(" | "));
...
void MainWindow::on_actionAdd_custom_triggered() {
  QString customExtensions = QInputDialog::getText(
      this, tr("Add custom file extensions"),
    ...
    if (match.hasMatch()) {
      if (match.captured().size() != ext.size()) {
        QMessageBox::critical(
            this, "Error", QString("Invalid extension %1, aborting.").arg(ext));
        return;
      }
...
void MainWindow::on_actionChoose_path_triggered() {
  if (!isConnected()) {
    client_logger->info(
        "Declining Choosing path and connection is not established.");
    QMessageBox::warning(this, "Connection failure.",
                         "Connection with server is not established.");
    return;
  }

  currentPath = QInputDialog::getText(this, tr("Choose path to be tracked"),
                                      tr("Enter path: "));
